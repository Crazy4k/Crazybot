const {guildsCache, pointsCache} = require("../caches/botCache");
...
module.exports = async (message, server) => { 
    ...
    const myPromise = new Promise(async (resolve, reject)=>{
        ...
        let servery = pointsCache[message.guild.id];
        ...
        if(servery === null){
            await mongo().then(async (mongoose) =>{
                try{
                    ...
                    servery = pointsCache[message.guild.id] = {
                        _id:message.guild.id,
                        whiteListedRole:"",
                        members:{},
        ...
        if(!server.pointsEnabled){
            ...
            pointsCache[message.guild.id] = servery;
