...
module.exports = async () =>{
  fs.readFile("./Private_JSON_files/raiderGroupsHistory.json","utf-8",async (err,config)=>{
    if(err) {
      console.log(err);
    ...
    fs.writeFile("./Private_JSON_files/raiderGroupsHistory.json",stringObj ,(err, config)=>{
      if(err) {
          console.log(err);
      }
      else {
          console.log("UPDATED RAIDER GROUPS HISTORY");
      }
    });
