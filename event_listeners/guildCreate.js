...
module.exports = async (guild, client) => {
    ...
    try {
		const serverObject = {
			...
			language:"English",
			overWriteDefaultPermission: false,
			onlySlash: false,
			prefix : ";",
			verifiedRole:"",
		...
		await mongo().then(async (mongoose) =>{
			try{ 
				await serversSchema.findOneAndUpdate({_id:guild.id},{
					...
					overWriteDefaultPermission: serverObject.overWriteDefaultPermission,
					onlySlash: serverObject.onlySlash,
					...
					robloxBinds : serverObject.robloxBinds,
