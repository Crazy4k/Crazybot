...
async function createJoinEmbed(customRaiderCache, userId){
    let thing = customRaiderCache[userId].split(" ");
...
module.exports = async function stalk( noblox, userIds, groupId, discordClient , channelId ){
    ...
    if(arrayOfChannels.length){
        ...
        if(!Object.values(customRaiderCache[groupId]).length){
            ...
            if(Object.values(customRaiderCache[groupId]).length){
                for(let I in customRaiderCache[groupId]){
                    if(customRaiderCache[groupId][I]){
                        
                        let embed = await createJoinEmbed(customRaiderCache[groupId], I)

                        changes.set(`${I}-${customRaiderCache[groupId][I]}`,embed);
                        joins.push(embed);
                    }
            ...
            if(joins.length){
                for(let id of arrayOfChannels){
                    ...
                    if(log){
                        let role = log.guild.roles.cache.find(e=>e.name === "c_raider_pings");
                        let ping = "@c_raider_pings";
            ...
            if(!Object.values(customRaiderCache[groupId]).length  && Object.values(newCache).length ) {
                for(let I in newCache){
                    if(!customRaiderCache[groupId][I] && newCache[I]){
            ...
            } else if(Object.values(customRaiderCache[groupId]).length && !Object.values(newCache).length ) {
                for(let I in customRaiderCache[groupId]){
                ...
                for(let I in customRaiderCache[groupId]){
                    if(customRaiderCache[groupId][I] && newCache[I] && customRaiderCache[groupId][I] !== newCache[I] ){
                    ...
                    else if(!customRaiderCache[groupId][I] && newCache[I]){
            ...
            customRaiderCache[groupId] = newCache;
            cache.customRaiderCache[groupId] = newCache;
