...
let pointsbatchAdd = new Command("pointsbatch-add");
...
pointsbatchAdd.execute =async function(message, args, server) { 
    try {
        ...
        for (let index = 1; index < batchArray.length + 1; index++) {
            ...
            switch (persona) {
                ...
                default:
                    ...
                    embed.addField( `**${index}) ✅ Success**`,`Successfully added ${pointsToGive} points to <@${usero.id}>!`, false);
